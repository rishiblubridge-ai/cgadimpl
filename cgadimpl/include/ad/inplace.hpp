 ...
 */
AG_NODISCARD bool ensure_inplace_value(const std::shared_ptr<Node>& node);
 ...
 */
AG_NODISCARD bool recompute_inplace(const std::shared_ptr<Node>& node);
 ...
 */
AG_NODISCARD size_t get_tensor_version(Node* node);
 ...
 *      false → otherwise.
 */
AG_NODISCARD bool has_alias(Node* node);
 ...
 */
AG_NODISCARD bool erase_snapshot(Node* node);
