...
template<class T>
concept Scalar = std::is_arithmetic_v<std::remove_cvref_t<T>> && !std::is_same_v<std::remove_cvref_t<T>, bool>;
 // exclude bool and accept numeric
}
namespace ag {
...
AG_NODISCARD Tensor forward_eval_node(Node* node);

// inline Value scalar_value(float x) {
//   auto n = std::make_shared<Node>();
//   n->value = Tensor(1,1);
//   n->value(0,0) = x;
//   n->requires_grad = false; // constant by default
//   return Value{std::move(n)};
// } // namespace ag

// // add
// template<Scalar S>
// inline Value add(const Value& a, S b) {
//   return add(a, scalar_value(static_cast<float>(b)));
// }
// template<Scalar S>
// inline Value add(S a, const Value& b) {
//   return add(scalar_value(static_cast<float>(a)), b);
// }

// // sub    
// template<Scalar S>
// inline Value sub(const Value& a, S b) {
//   return sub(a, scalar_value(static_cast<float>(b)));
// }
// template<Scalar S>
// inline Value sub(S a, const Value& b) {
//   return sub(scalar_value(static_cast<float>(a)), b);
// }

// // mul
// template<Scalar S>
// inline Value mul(const Value& a, S b) {
//   return mul(a, scalar_value(static_cast<float>(b)));
// }
// template<Scalar S>
// inline Value mul(S a, const Value& b) {
//   return mul(scalar_value(static_cast<float>(a)), b);
// }

// // div
// template<Scalar S>
// inline Value div(const Value& a, S b) {
//   return div(a, scalar_value(static_cast<float>(b)));
// }
// template<Scalar S>
// inline Value div(S a, const Value& b) {
//   return div(scalar_value(static_cast<float>(a)), b);
// } 

// template<Scalar S> inline Value operator+(const Value& a, S b){ return add(a,b); }
// template<Scalar S> inline Value operator+(S a, const Value& b){ return add(a,b); }
// template<Scalar S> inline Value operator-(const Value& a, S b){ return sub(a,b); }
// template<Scalar S> inline Value operator-(S a, const Value& b){ return sub(a,b); }
// template<Scalar S> inline Value operator*(const Value& a, S b){ return mul(a,b); }
// template<Scalar S> inline Value operator*(S a, const Value& b){ return mul(a,b); }
// template<Scalar S> inline Value operator/(const Value& a, S b){ return div(a,b); }
// template<Scalar S> inline Value operator/(S a, const Value& b){ return div(a,b); }
